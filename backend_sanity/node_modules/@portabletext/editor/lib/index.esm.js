...
const debug$l = debugWithName("components:DraggableBlock"), DraggableBlock = (t0) => {
  ...
  let t19;
  return $[41] !== isVoid || $[42] !== handleDragStart || $[43] !== handleDrag || $[44] !== handleDragOver || $[45] !== handleDragEnd || $[46] !== handleDrop || $[47] !== t17 || $[48] !== children || $[49] !== t18 ? (t19 = /* @__PURE__ */ jsxs("div", { draggable: isVoid, onDragStart: handleDragStart, onDrag: handleDrag, onDragOver: handleDragOver, onDragLeave: handleDragLeave, onDragEnd: handleDragEnd, onDrop: handleDrop, children: [
    t17,
    children,
    t18
  ...
  return function(editor) {
    return portableTextEditor.setEditable({
      ...
      insertChild: (type, value) => {
        ...
        }], portableTextEditor)[0].children[0], focusChildPath = editor.selection.focus.path.slice(0, 2), isSpanNode = child._type === types.span.name, focusNode = Node.get(editor, focusChildPath);
      ...
      isAnnotationActive: (annotationType) => {
        ...
        try {
          ...
          return spans.every(([span]) => isPortableTextSpan$1(span) ? span.marks?.map((markKey) => selectionMarkDefs.find((def) => def?._key === markKey)?._type)?.includes(annotationType) : !1);
        } catch {
          return !1;
        }
      },
        ...
        if (originalSelection && (Range.isCollapsed(originalSelection) && (editor.pteExpandToWord(), editor.onChange()), editor.selection)) {
          ...
          Editor.withoutNormalizing(editor, () => {
            ...
            for (const [block, blockPath] of selectedBlocks) {
              ...
              markDefs.find((markDef) => markDef._type === type.name && markDef._key === annotationKey) === void 0 && (Transforms.setNodes(editor, {
                markDefs: [...markDefs, {
                  _type: type.name,
                  _key: annotationKey,
                  ...value
        ...
        debug$j("Removing annotation", type), Editor.withoutNormalizing(editor, () => {
          if (editor.selection)
            if (Range.isCollapsed(editor.selection)) {
              ...
              if (!editor.isTextBlock(block))
                return;
              const potentialAnnotations = (block.markDefs ?? []).filter((markDef) => markDef._type === type.name), [selectedChild, selectedChildPath] = Editor.node(editor, editor.selection, {
                depth: 2
              });
...
function insertPatch(editor, patch, schemaTypes) {
  ...
  if (patch.path.length === 1) {
    ...
    return debug$i(`Inserting blocks at path [${normalizedIdx2}]`), debugState(editor, "before"), Transforms.insertNodes(editor, blocksToInsert, {
      at: [normalizedIdx2]
...
function unsetPatch(editor, patch) {
  ...
  if (patch.path.length === 1) {
    ...
    return debug$i(`Removing block at path [${blockIndex}]`), debugState(editor, "before"), Transforms.removeNodes(editor, {
      at: [blockIndex]
    }), debugState(editor, "after"), !0;
  }
  return editor.isTextBlock(block) && patch.path[1] === "children" && patch.path.length === 3 ? !child || !childPath ? (debug$i("Child not found"), !1) : (debug$i(`Unsetting child at path ${JSON.stringify(childPath)}`), debugState(editor, "before"), debugVerbose$4 && debug$i(`Removing child at path ${JSON.stringify(childPath)}`), Transforms.removeNodes(editor, {
    at: childPath
    ...
    return editor.apply = (op) => {
      ...
      if (op.type === "remove_node") {
        ...
        if (op.path[0] === 0 && Editor.isVoid(editor, node)) {
          ...
          editor.children[nextPath[0]] || (debug$f("Adding placeholder block"), Editor.insertNode(editor, editor.pteCreateTextBlock({
            decorators: []
          })));
        }
      }
  ...
  return function(editor) {
    ...
    return editor.normalizeNode = (nodeEntry) => {
      ...
      editor.selection && ([...Editor.nodes(editor, {
        ...
        editor.isTextBlock(node) && node.style === blockStyle ? (debug$e(`Unsetting block style '${blockStyle}'`), Transforms.setNodes(editor, {
          ...node,
          style: defaultStyle
        }, {
          at: path
        })) : (blockStyle ? debug$e(`Setting style '${blockStyle}'`) : debug$e("Setting default style", defaultStyle), Transforms.setNodes(editor, {
          ...node,
    ...
    return editor.pteToggleListItem = (listItemStyle) => {
      ...
      editor.selection && [...Editor.nodes(editor, {
        ...
        if (editor.isListBlock(node)) {
          ...
          debug$d(`Unsetting list '${listItemStyle}'`), Transforms.setNodes(editor, newNode, {
            at: path
          });
        }
      });
      ...
      editor.selection && [...Editor.nodes(editor, {
        ...
        debug$d(`Setting list '${listItemStyle}'`), Transforms.setNodes(editor, {
          ...node,
          level: 1,
      ...
      return selectedBlocks.length === 0 ? !1 : (selectedBlocks.forEach(([node, path]) => {
        Element$1.isElement(node) && (debug$d("Unset list"), Transforms.setNodes(editor, {
          ...node,
          level: void 0,
          listItem: void 0
        }, {
      ...
      return selectedBlocks.length === 0 ? !1 : (selectedBlocks.forEach(([node, path]) => {
        if (editor.isListBlock(node)) {
          ...
          reverse ? (level--, debug$d("Decrementing list level", Math.min(MAX_LIST_LEVEL, Math.max(1, level)))) : (level++, debug$d("Incrementing list level", Math.min(MAX_LIST_LEVEL, Math.max(1, level)))), Transforms.setNodes(editor, {
...
function createWithPortableTextMarkModel(editorActor, types) {
  return function(editor) {
    const {
      ...
      editor.selection && (Transforms.select(editor, {
        ...editor.selection
      }), editor.selection = {
        ...editor.selection
      });
...
function _insertFragment(editor, fragment, schemaTypes) {
  editor.withoutNormalizing(() => {
    ...
    if (editor.isTextBlock(focusBlock) && editor.isTextBlock(fragment[0])) {
      ...
      debug$7("Mixing markDefs of focusBlock and fragments[0] block", markDefs, fragment[0].markDefs), isEqual(markDefs, fragment[0].markDefs) || Transforms.setNodes(editor, {
              ...
              withoutPatching(slateEditor, () => {
                ...
                slateValueFromProps.forEach((currentBlock, currentBlockIndex) => {
                  ...
                  if (!oldBlock && isValid) {
                    ...
                    debug$5.enabled && debug$5("Validating and inserting new block in the end of the value", currentBlock), validation_0.valid || validation_0.resolution?.autoResolve ? Transforms.insertNodes(slateEditor, currentBlock, {
...
const EditorActorContext = createContext({}), insertBreakActionImplementation = ({
  ...
  select: ({
    ...
    Transforms.select(action.editor, toSlateRange(action.selection, action.editor));
  }
};
...
function PortableTextEditorSelectionProvider(props) {
  ...
  let t2;
  return $[3] !== selection || $[4] !== props.children ? (t2 = /* @__PURE__ */ jsx(PortableTextEditorSelectionContext.Provider, { value: selection, children: props.children }), $[3] = selection, $[4] = props.children, $[5] = t2) : t2 = $[5], t2;
}
