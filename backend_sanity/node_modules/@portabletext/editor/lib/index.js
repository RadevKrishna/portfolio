...
const debug$l = debugWithName("components:DraggableBlock"), DraggableBlock = (t0) => {
  ...
  let t19;
  return $[41] !== isVoid || $[42] !== handleDragStart || $[43] !== handleDrag || $[44] !== handleDragOver || $[45] !== handleDragEnd || $[46] !== handleDrop || $[47] !== t17 || $[48] !== children || $[49] !== t18 ? (t19 = /* @__PURE__ */ jsxRuntime.jsxs("div", { draggable: isVoid, onDragStart: handleDragStart, onDrag: handleDrag, onDragOver: handleDragOver, onDragLeave: handleDragLeave, onDragEnd: handleDragEnd, onDrop: handleDrop, children: [
    t17,
    children,
    t18
  ...
  return function(editor) {
    return portableTextEditor.setEditable({
      ...
      focusChild: () => {
        if (editor.selection) {
          ...
          if (block && editor.isTextBlock(block))
            return fromSlateValue([block], types$1.block.name, KEY_TO_VALUE_ELEMENT.get(editor))[0].children[editor.selection.focus.path[1]];
        }
      },
      insertChild: (type, value) => {
        ...
        }], portableTextEditor)[0].children[0], focusChildPath = editor.selection.focus.path.slice(0, 2), isSpanNode = child._type === types$1.span.name, focusNode = slate.Node.get(editor, focusChildPath);
        return isSpanNode && focusNode._type !== types$1.span.name && (debug$j("Inserting span child next to inline object child, moving selection + 1"), editor.move({
          distance: 1,
          unit: "character"
        })), slate.Transforms.insertNodes(editor, child, {
          select: !0,
          at: editor.selection
        }), editor.onChange(), toPortableTextRange(fromSlateValue(editor.children, types$1.block.name, KEY_TO_VALUE_ELEMENT.get(editor)), editor.selection, types$1)?.focus.path || [];
      },
      insertBlock: (type, value) => {
        ...
        if (!editor.selection) {
          ...
          return slate.Editor.insertNode(editor, block), lastBlock && isEqualToEmptyEditor([lastBlock[0]], types$1) && slate.Transforms.removeNodes(editor, {
            at: lastBlock[1]
          }), editor.onChange(), toPortableTextRange(fromSlateValue(editor.children, types$1.block.name, KEY_TO_VALUE_ELEMENT.get(editor)), editor.selection, types$1)?.focus.path ?? [];
        }
        const focusBlock = Array.from(slate.Editor.nodes(editor, {
        ...
        return slate.Editor.insertNode(editor, block), focusBlock && isEqualToEmptyEditor([focusBlock[0]], types$1) && slate.Transforms.removeNodes(editor, {
          at: focusBlock[1]
        }), editor.onChange(), toPortableTextRange(fromSlateValue(editor.children, types$1.block.name, KEY_TO_VALUE_ELEMENT.get(editor)), editor.selection, types$1)?.focus.path || [];
      },
      hasBlockStyle: (style) => {
        ...
        if (slatePath) {
          ...
          if (block && blockPath && typeof block._key == "string") {
            if (path.length === 1 && slatePath.focus.path.length === 1)
              return [fromSlateValue([block], types$1.block.name)[0], [{
                _key: block._key
              }]];
            const ptBlock = fromSlateValue([block], types$1.block.name, KEY_TO_VALUE_ELEMENT.get(editor))[0];
      ...
      isAnnotationActive: (annotationType) => {
        ...
        try {
          ...
          return spans.every(([span]) => types.isPortableTextSpan(span) ? span.marks?.map((markKey) => selectionMarkDefs.find((def) => def?._key === markKey)?._type)?.includes(annotationType) : !1);
        } catch {
          return !1;
        }
      },
        ...
        if (originalSelection && (slate.Range.isCollapsed(originalSelection) && (editor.pteExpandToWord(), editor.onChange()), editor.selection)) {
          ...
          slate.Editor.withoutNormalizing(editor, () => {
            ...
            for (const [block, blockPath] of selectedBlocks) {
              ...
              markDefs.find((markDef) => markDef._type === type.name && markDef._key === annotationKey) === void 0 && (slate.Transforms.setNodes(editor, {
                markDefs: [...markDefs, {
                  _type: type.name,
                  _key: annotationKey,
                  ...value
        ...
        debug$j("Removing annotation", type), slate.Editor.withoutNormalizing(editor, () => {
          if (editor.selection)
            if (slate.Range.isCollapsed(editor.selection)) {
              ...
              if (!editor.isTextBlock(block))
                return;
              const potentialAnnotations = (block.markDefs ?? []).filter((markDef) => markDef._type === type.name), [selectedChild, selectedChildPath] = slate.Editor.node(editor, editor.selection, {
                depth: 2
              });
      ...
      getSelection: () => {
        ...
        if (editor.selection) {
          ...
          if (existing)
            return existing;
          ptRange = toPortableTextRange(fromSlateValue(editor.children, types$1.block.name, KEY_TO_VALUE_ELEMENT.get(editor)), editor.selection, types$1), SLATE_TO_PORTABLE_TEXT_RANGE.set(editor.selection, ptRange);
        }
        return ptRange;
      },
...
function insertPatch(editor, patch, schemaTypes) {
  ...
  if (patch.path.length === 1) {
    ...
    return debug$i(`Inserting blocks at path [${normalizedIdx2}]`), debugState(editor, "before"), slate.Transforms.insertNodes(editor, blocksToInsert, {
      at: [normalizedIdx2]
...
function unsetPatch(editor, patch) {
  ...
  if (patch.path.length === 1) {
    ...
    return debug$i(`Removing block at path [${blockIndex}]`), debugState(editor, "before"), slate.Transforms.removeNodes(editor, {
      at: [blockIndex]
    }), debugState(editor, "after"), !0;
  }
    ...
    return editor.apply = (op) => {
      ...
      if (op.type === "remove_node") {
        ...
        if (op.path[0] === 0 && slate.Editor.isVoid(editor, node)) {
          ...
          editor.children[nextPath[0]] || (debug$f("Adding placeholder block"), slate.Editor.insertNode(editor, editor.pteCreateTextBlock({
            decorators: []
          })));
        }
      }
  ...
  return function(editor) {
    ...
    return editor.normalizeNode = (nodeEntry) => {
      ...
      editor.selection && ([...slate.Editor.nodes(editor, {
        ...
        editor.isTextBlock(node) && node.style === blockStyle ? (debug$e(`Unsetting block style '${blockStyle}'`), slate.Transforms.setNodes(editor, {
          ...node,
          style: defaultStyle
        }, {
          at: path
        })) : (blockStyle ? debug$e(`Setting style '${blockStyle}'`) : debug$e("Setting default style", defaultStyle), slate.Transforms.setNodes(editor, {
          ...node,
    ...
    return editor.pteToggleListItem = (listItemStyle) => {
      ...
      editor.selection && [...slate.Editor.nodes(editor, {
        ...
        if (editor.isListBlock(node)) {
          ...
          debug$d(`Unsetting list '${listItemStyle}'`), slate.Transforms.setNodes(editor, newNode, {
            at: path
          });
        }
      });
      ...
      editor.selection && [...slate.Editor.nodes(editor, {
        ...
        debug$d(`Setting list '${listItemStyle}'`), slate.Transforms.setNodes(editor, {
          ...node,
          level: 1,
      ...
      return selectedBlocks.length === 0 ? !1 : (selectedBlocks.forEach(([node, path]) => {
        slate.Element.isElement(node) && (debug$d("Unset list"), slate.Transforms.setNodes(editor, {
          ...node,
          level: void 0,
          listItem: void 0
        }, {
      ...
      return selectedBlocks.length === 0 ? !1 : (selectedBlocks.forEach(([node, path]) => {
        if (editor.isListBlock(node)) {
          ...
          reverse ? (level--, debug$d("Decrementing list level", Math.min(MAX_LIST_LEVEL, Math.max(1, level)))) : (level++, debug$d("Incrementing list level", Math.min(MAX_LIST_LEVEL, Math.max(1, level)))), slate.Transforms.setNodes(editor, {
...
function createWithPortableTextMarkModel(editorActor, types2) {
  return function(editor) {
    const {
      apply: apply2,
      normalizeNode
    } = editor, decorators = types2.decorators.map((t) => t.value), forceNewSelection = () => {
      editor.selection && (slate.Transforms.select(editor, {
        ...editor.selection
      }), editor.selection = {
        ...editor.selection
      });
...
function _insertFragment(editor, fragment, schemaTypes) {
  editor.withoutNormalizing(() => {
    ...
    if (editor.isTextBlock(focusBlock) && editor.isTextBlock(fragment[0])) {
      ...
      debug$7("Mixing markDefs of focusBlock and fragments[0] block", markDefs, fragment[0].markDefs), isEqual__default.default(markDefs, fragment[0].markDefs) || slate.Transforms.setNodes(editor, {
              ...
              withoutPatching(slateEditor, () => {
                ...
                slateValueFromProps.forEach((currentBlock, currentBlockIndex) => {
                  ...
                  if (!oldBlock && isValid) {
                    ...
                    debug$5.enabled && debug$5("Validating and inserting new block in the end of the value", currentBlock), validation_0.valid || validation_0.resolution?.autoResolve ? slate.Transforms.insertNodes(slateEditor, currentBlock, {
...
const EditorActorContext = require$$0.createContext({}), insertBreakActionImplementation = ({
  ...
  select: ({
    ...
    slate.Transforms.select(action.editor, toSlateRange(action.selection, action.editor));
  }
};
